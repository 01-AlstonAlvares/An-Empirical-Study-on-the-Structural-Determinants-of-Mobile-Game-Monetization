...
#### Step 1: Multi-Dataset Ingestion, Scoping Audit & Archetype Formulation
* **Notebook:** `notebooks/01_data_ingestion_and_labeling.ipynb`
* **Raw Candidate Corpus Ingested:** Three open-access storefront datasets evaluated in `data/raw/`:

| Candidate Dataset | Source / Provenance | Ingested Records | Content Scope | In-App Purchase Granularity | Feasibility & Scoping Decision |
| :--- | :--- | :---: | :--- | :--- | :--- |
| **1. Apple App Store Strategy Games** (`appstore_games.csv`) | [tristan581 / Kaggle](https://www.kaggle.com/datasets/tristan581/17k-apple-app-store-strategy-games) (CC0) | **17,007** | **100% Mobile Strategy Games** | **Itemized Price Strings** (`$0.99` to `$99.99`) | **SELECTED (Primary Modeling Cohort):** Only dataset with granular IAP price arrays required to operationalize the ground-truth target ($Max\_IAP \ge \$19.99$). |
| **2. Google Play Store 10K Apps** (`googleplaystore.csv`) | [lava18 / Kaggle](https://www.kaggle.com/datasets/lava18/google-play-store-apps) (CC0) | 10,841 | General Storefront (87% Non-Gaming) | **None** (Only binary Free/Paid and upfront price) | **Excluded from Primary Model:** Lacks IAP tier distributions; dominated by non-game software (tools, social, productivity). |
| **3. Google Play Store 2.3M Apps** (`Google-Playstore.csv`) | [gauthamp10 / Kaggle](https://www.kaggle.com/datasets/gauthamp10/google-playstore-apps) (CC0) | 2,312,944 | General Storefront (85%+ Non-Gaming) | **Coarse Boolean Only** (`In App Purchases: True/False`) | **Excluded from Primary Model:** Boolean flag cannot distinguish a $0.99 tip from a $99.99 whale loop; violates target operationalization. |

**Dataset Scoping Rationale:**  
While three datasets were ingested and audited during data engineering, primary modeling was specifically scoped to the Apple App Store Strategy Games cohort for three methodological reasons:
1. *Target Feasibility:* Only `appstore_games.csv` records granular in-app purchase price arrays (`$0.99` to `$99.99`) required to mathematically compute $Max\_IAP \ge \$19.99$. The Google Play datasets lack itemized IAP price distributions (only coarse boolean flags or binary Free/Paid indicators).
2. *Domain Homogeneity:* Maya is greenlighting a *game*. Over 85% of records in the Google Play datasets represent non-gaming software (utilities, social, business). Strategy games represent the mechanically richest gaming genre where backend architecture directly governs monetization.
3. *Platform Confounding Prevention:* Cross-store pooling introduces severe reporting artifacts (e.g., Android 100 MB split APK limits vs iOS 150 MB+ bundle limits) that would cause the model to explain storefront differences rather than core game design mechanics.
