...
## 2. Multi-Dataset Ingestion & Scoping Audit

The project repository includes three candidate open-access storefront datasets in `data/raw/`. During Phase 1 (Data Ingestion & Scoping), we conducted a feasibility audit across all three sources:

| Candidate Dataset | Source & Provenance | Records | Domain / Content Scope | In-App Purchase Granularity | Scoping Decision & Rationale |
| :--- | :--- | :---: | :--- | :--- | :--- |
| **1. Apple App Store Strategy Games** (`appstore_games.csv`) | [tristan581 / Kaggle](https://www.kaggle.com/datasets/tristan581/17k-apple-app-store-strategy-games) (CC0) | **17,007** | **100% Strategy & Tactical Games** | **Itemized Price Tiers** (`$0.99` to `$99.99`) | **SELECTED (Primary Modeling Cohort):** Only dataset providing granular IAP prices required to operationalize the ground-truth target ($Max\_IAP \ge \$19.99$). |
| **2. Google Play Store 10K Apps** (`googleplaystore.csv`) | [lava18 / Kaggle](https://www.kaggle.com/datasets/lava18/google-play-store-apps) (CC0) | 10,841 | General Storefront (87% Non-Gaming Apps) | **None** (Only binary Free/Paid and upfront price) | **Excluded from Primary Model:** Lacks IAP tier distributions; dominated by non-game software (tools, social, productivity). |
| **3. Google Play Store 2.3M Apps** (`Google-Playstore.csv`) | [gauthamp10 / Kaggle](https://www.kaggle.com/datasets/gauthamp10/google-playstore-apps) (CC0) | 2,312,944 | General Storefront (85%+ Non-Gaming Apps) | **Coarse Boolean Only** (`In App Purchases: True/False`) | **Excluded from Primary Model:** Boolean flag cannot distinguish a $0.99 tip from a $99.99 whale loop; violates target operationalization. |

### Why Focus on Strategy Games and the iOS Storefront?
1. **Target Variable Operationalization ($Y$):**  
   The project's ground-truth target distinguishes between Aggressive IAP ($Y=1$: $Max\_IAP \ge \$19.99$) and Ad-Supported Freemium ($Y=0$: $Max\_IAP < \$4.99$). Only `appstore_games.csv` records the itemized string of in-app purchase prices.
2. **Operational Relevance to Lead Producer Maya:**  
   Maya is greenlighting a *game*, not a flashlight utility or calendar app. Strategy games represent the highest-grossing, mechanically richest genre where backend decisions (synchronous real-time multiplayer vs. turn-based loops, asset download footprints, and live-ops cadence) directly govern monetization economics.
3. **Preventing Cross-Platform Confounding:**  
   Google Play and Apple App Store have incompatible packaging architectures (e.g., Google Play's historical 100 MB split APK limit vs. Apple's 150 MB+ OTA cellular download limit) and disjoint age-rating statutory frameworks. Pooling them naively would train the explainer to identify storefront platform quirks rather than gameplay mechanics.

### Feature Leakage Prevention
Financial fields (`Price`, `In-app Purchases`, `max_iap_price`) are strictly sequestered to formulate the ground-truth target label $Y$ and are deleted from the feature matrix $X$ before model training.

### Data Ingestion Setup
1. Raw datasets are stored in `data/raw/` (`appstore_games.csv`, `googleplaystore.csv`, `Google-Playstore.csv`).
2. Run `notebooks/01_data_ingestion_and_labeling.ipynb` to parse heterogeneous storage notations into continuous megabytes, isolate 4,071 ambiguous mid-tier entries ($4.99–$19.99), and generate stratified splits (`data/processed/train.parquet` and `data/processed/test.parquet`).
...
```
├── data/
│   ├── raw/                              # Raw store CSVs (App Store strategy games + Google Play benchmark datasets)
